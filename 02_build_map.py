...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .nav-title { font-weight: 700; font-size: 15px; letter-spacing: 0.12em; color: var(--text); display: flex; align-items: baseline; gap: 8px; cursor: pointer; }
  .nav-title:hover { color: var(--amber); }
  ...
  .zoning-toggle {
    position: absolute; top: 54px; left: 20px; z-index: 1000;
    background: rgba(255,255,255,0.92); border: 1px solid var(--panel-border); border-radius: 3px;
    padding: 6px 12px; font-size: 10px; letter-spacing: 0.06em; text-transform: uppercase;
    color: var(--text-muted); cursor: pointer; user-select: none;
  }
  .zoning-toggle.on { color: #7b3fd4; border-color: #7b3fd4; }

  .address-finder {
    position: absolute; top: 92px; left: 20px; z-index: 1000;
    background: rgba(255,255,255,0.92); border: 1px solid var(--panel-border); border-radius: 3px;
  ...
  .address-finder input {
    ...
    border: 1px solid var(--panel-border); border-radius: 2px; padding: 5px 8px; width: 150px; background: #fff;
  }
  .address-finder input:focus { outline: none; border-color: var(--amber-dim); }
  .address-finder button {
    font-family: 'Courier New', monospace; font-size: 10px; letter-spacing: 0.06em; text-transform: uppercase;
    background: var(--panel); color: var(--text); border: 1px solid var(--panel-border); border-radius: 2px;
    padding: 6px 10px; cursor: pointer;
  }
  .address-finder button:hover { color: var(--amber); border-color: var(--amber-dim); }
  .address-finder-status { position: absolute; top: 132px; left: 20px; z-index: 1000; font-size: 10px; color: var(--text-muted); background: rgba(255,255,255,0.92); border: 1px solid var(--panel-border); border-radius: 3px; padding: 4px 10px; display: none; }
  ...
  .notify-wrap { padding: 24px; max-width: 640px; margin: 0 auto; }
  .notify-wrap h2 { font-size: 14px; letter-spacing: 0.08em; text-transform: uppercase; color: var(--text); margin: 0 0 10px; }
  .notify-wrap p { color: var(--text-muted); font-size: 13px; margin: 0 0 16px; }
  .notify-form-frame { width: 100%; height: 900px; border: 1px solid var(--panel-border); border-radius: 4px; background: #fff; }
  @media (max-width: 640px) { .notify-form-frame { height: 1100px; } }
...
<div id="navbar">
  ...
  <div class="nav-tabs">
    ...
    <button class="nav-tab" data-view="notify">Get Notified</button>
  </div>
</div>
...
<div class="zoning-toggle" id="zoningToggle">Show zoning districts</div>
<div class="address-finder">
  <input type="text" id="addressInput" placeholder="Find your address..." />
  <button id="addressFindBtn">Go</button>
</div>
<div class="address-finder-status" id="addressFinderStatus"></div>
...
<div id="view-notify" class="view">
  <div class="notify-wrap">
    <h2>Get notified about cases near you</h2>
    <p>Enter your email and address below to get an email whenever a new development case shows up within your chosen radius. You can also <a href="https://docs.google.com/forms/d/e/1FAIpQLSeJwpWv1NKkoG3toVPOFo2YBBi_hd0JwwqCUsYzBIgZYEM2SA/viewform" target="_blank" rel="noopener" style="color:var(--teal);">open the form in a new tab</a> if it doesn't display below.</p>
    <iframe class="notify-form-frame" src="https://docs.google.com/forms/d/e/1FAIpQLSeJwpWv1NKkoG3toVPOFo2YBBi_hd0JwwqCUsYzBIgZYEM2SA/viewform?embedded=true" loading="lazy">Loading form…</iframe>
  </div>
</div>
...
<script>
  ...
  var VALID_VIEWS = ['map', 'table', 'stats', 'about', 'notify'];

  function activateView(viewName, updateHash) {
    if (VALID_VIEWS.indexOf(viewName) === -1) viewName = 'map';
  ...
  document.querySelectorAll('.nav-tab').forEach(function (tab) {
    tab.addEventListener('click', function () { activateView(tab.getAttribute('data-view'), true); });
  });

  document.getElementById('navTitle').addEventListener('click', function () { activateView('map', true); });
  ...
  function initMap() {
    ...
    try {
      ...
      // Zoning district overlay: Raleigh's own public zoning GIS layer,
      // added lazily (only once the user actually asks to see it) as a
      // dynamic raster export -- MapLibre substitutes {bbox-epsg-3857} per
      // tile automatically, so we only ever fetch what's in view rather
      // than pulling every zoning polygon in the city up front.
      var ZONING_EXPORT_URL =
        'https://maps.raleighnc.gov/arcgis/rest/services/Planning/Zoning/MapServer/export' +
        '?bbox={bbox-epsg-3857}&size=256,256&imageSR=3857&bboxSR=3857&format=png32&transparent=true&f=image&layers=show:0';
      var zoningLoaded = false;
      var zoningOn = false;

      function addZoningLayer() {
        map.addSource('zoning', { type: 'raster', tiles: [ZONING_EXPORT_URL], tileSize: 256 });
        map.addLayer({ id: 'zoning-layer', type: 'raster', source: 'zoning', paint: { 'raster-opacity': 0.45 } });
        zoningLoaded = true;
      }

      document.getElementById('zoningToggle').addEventListener('click', function () {
        zoningOn = !zoningOn;
        this.classList.toggle('on', zoningOn);

        if (!zoningLoaded) {
          if (map.isStyleLoaded()) {
            addZoningLayer();
          } else {
            map.once('load', addZoningLayer); // style wasn't ready yet -- wait for it
          }
        } else {
          map.setLayoutProperty('zoning-layer', 'visibility', zoningOn ? 'visible' : 'none');
        }
      });
      ...
      // "Find my address" -- uses OpenStreetMap's free Nominatim geocoder
      // (not our Geoapify key) since this code runs in every visitor's own
      // browser; a paid key embedded client-side would be visible in the
      // page source and could be copied and abused by anyone.
      var userAddressMarker = null;
      var addressStatusEl = document.getElementById('addressFinderStatus');

      function showAddressStatus(msg, isError) {
        addressStatusEl.textContent = msg;
        addressStatusEl.style.display = msg ? 'block' : 'none';
        addressStatusEl.style.color = isError ? '#d62839' : 'var(--text-muted)';
      }

      function findMyAddress() {
        var query = document.getElementById('addressInput').value.trim();
        if (!query) return;

        showAddressStatus('Searching…', false);

        var url = 'https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=us' +
          '&viewbox=-79.0,36.1,-78.3,35.5&bounded=1&q=' + encodeURIComponent(query + ', Raleigh, NC');

        fetch(url)
          .then(function (r) { return r.json(); })
          .then(function (results) {
            if (!results || !results.length) {
              showAddressStatus('Address not found — try adding more detail.', true);
              return;
            }
            var lat = parseFloat(results[0].lat), lng = parseFloat(results[0].lon);

            if (userAddressMarker) userAddressMarker.remove();
            var el = document.createElement('div');
            el.style.width = '18px';
            ...
            userAddressMarker = new maplibregl.Marker({ element: el }).setLngLat([lng, lat]).addTo(map);

            map.flyTo({ center: [lng, lat], zoom: 15, duration: 800 });
            showAddressStatus('', false);
          })
          .catch(function (err) {
            console.error(err);
            showAddressStatus('Lookup failed — try again in a moment.', true);
          });
      }

      document.getElementById('addressFindBtn').addEventListener('click', findMyAddress);
      document.getElementById('addressInput').addEventListener('keydown', function (e) {
        if (e.key === 'Enter') findMyAddress();
      });
  ...
  // Restore whichever tab was active before a refresh (via the URL hash),
  // defaulting to Map if there's no hash or it doesn't match a real view.
  // The map itself only initializes here if it's actually the resolved
  // initial view -- no point loading MapLibre + fetching tiles if the
  // visitor is landing directly on, say, the About page.
  var initialView = window.location.hash ? window.location.hash.slice(1) : 'map';
